...
internal class OneTryUpdatesListenerTest {
    ...
    private val listener = OneTryUpdatesListener(dispatcher)
    ...
    fun `should confirm all if failed during dispatch`() {
        every { dispatcher.dispatch(u1) } throws Exception()
    ...
    fun `should apply pre-check`() {
        val rs = OneTryUpdatesListener(dispatcher, preCheck = { false })
